...
One strategy to follow when playing Scrabble is to exploit the fact that som letters are worth more than others. Letters with higher scores ar less common in the language and therefore more difficult to use. Naturally, it is preferable to use them as soon as possible, to not risk difficult situations later in the game. The HSW player prefers placing words including high score letters. By all legal moves generated, the HSW player will choose the one where the word itself has the highest score. The HSW algorithm is as follows:
...
\bibitem{dictionary} Swedish dictionary Den stora svenska ordlistan, http://code.google.com/p/dsso/downloads/detail?name=dsso-1.52.txt\&can=1\&q=
